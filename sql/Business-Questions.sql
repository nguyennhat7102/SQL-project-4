...
SELECT
	t.customer_key ,
	t.order_date ,
	LAG(order_date) OVER(PARTITION BY customer_key ORDER BY order_date ASC) AS pre_purchase
FROM
	fact_sales t
GROUP BY
	t.customer_key ,
...
SELECT
	* ,
	( order_date - pre_purchase 
) AS days_between_purchase
FROM
	repurchase
WHERE
	pre_purchase IS NOT NULL 
)
SELECT
	percentile_cont(0.5) WITHIN GROUP (
	ORDER BY days_between_purchase) AS median_repurchase,
	stddev(days_between_purchase) AS std_repurchase
FROM
	tmp
...
FROM fact_sales
JOIN first_purchase USING(customer_key)
...
SELECT
    ...
    MAX(CASE WHEN cohort_age = 0 THEN retention_rate END) AS M0,
    MAX(CASE WHEN cohort_age = 1 THEN retention_rate END) AS M1,
    MAX(CASE WHEN cohort_age = 2 THEN retention_rate END) AS M2,
    MAX(CASE WHEN cohort_age = 3 THEN retention_rate END) AS M3,
    MAX(CASE WHEN cohort_age = 4 THEN retention_rate END) AS M4,
    MAX(CASE WHEN cohort_age = 5 THEN retention_rate END) AS M5,
    MAX(CASE WHEN cohort_age = 6 THEN retention_rate END) AS M6,
    MAX(CASE WHEN cohort_age = 7 THEN retention_rate END) AS M7,
    MAX(CASE WHEN cohort_age = 8 THEN retention_rate END) AS M8,
    MAX(CASE WHEN cohort_age = 9 THEN retention_rate END) AS M9,
    MAX(CASE WHEN cohort_age = 10 THEN retention_rate END) AS M10,
    MAX(CASE WHEN cohort_age = 11 THEN retention_rate END) AS M11
...
SELECT
	t.customer_key,
	date_trunc('month', MIN(t.order_date))::date AS cohort_month
FROM
	fact_sales t
WHERE
	EXTRACT(YEAR FROM t.order_date ) = 2022
GROUP BY
	t.customer_key 
),
repuschase AS (
SELECT
	DISTINCT t.customer_key,
...
SELECT
	*,
	EXTRACT(MONTH FROM age(order_date, cohort_month)) AS cohort_age
FROM
	repuschase
...
SELECT
	cohort_month ,
	cohort_age,
	count( DISTINCT customer_key ) AS retained_customers -- distinct bởi vì sẽ có khách hàng trong một tháng mua nhiều lần thì ta chỉ tính một lần mua thôi 
FROM
	cohort_age
GROUP BY
	cohort_month ,
...
, cohort_size AS (
SELECT cohort_month,
max(retained_customers) AS cohort_volumn
FROM retention_count
...
SELECT
	cohort_month,
	MAX(CASE WHEN cohort_age = 0 THEN round(retained_customers * 100.0 / cohort_volumn,2)  END) AS M0,
	MAX(CASE WHEN cohort_age = 1 THEN round(retained_customers * 100.0 / cohort_volumn ,2) END) AS M1,
	MAX(CASE WHEN cohort_age = 2 THEN round(retained_customers * 100.0 / cohort_volumn,2) END) AS M2,
	MAX(CASE WHEN cohort_age = 3 THEN round(retained_customers * 100.0 / cohort_volumn,2) END) AS M3,
	MAX(CASE WHEN cohort_age = 4 THEN round(retained_customers * 100.0 / cohort_volumn ,2)  END) AS M4,
	MAX(CASE WHEN cohort_age = 5 THEN round(retained_customers * 100.0 / cohort_volumn,2)  END) AS M5,
	MAX(CASE WHEN cohort_age = 6 THEN round(retained_customers * 100.0 / cohort_volumn,2) END) AS M6,
	MAX(CASE WHEN cohort_age = 7 THEN round(retained_customers * 100.0 / cohort_volumn,2)  END) AS M7,
	MAX(CASE WHEN cohort_age = 8 THEN round(retained_customers * 100.0 / cohort_volumn,2) END) AS M8,
	MAX(CASE WHEN cohort_age = 9 THEN round(retained_customers * 100.0 / cohort_volumn,2)  END) AS M9,
	MAX(CASE WHEN cohort_age = 10 THEN round(retained_customers * 100.0 / cohort_volumn,2)  END) AS M10,
	MAX(CASE WHEN cohort_age = 11 THEN round(retained_customers * 100.0 / cohort_volumn,2) END) AS M11
FROM
	tmp
GROUP BY
	cohort_month
